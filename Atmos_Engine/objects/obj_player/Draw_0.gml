...
#region Draw Hookshot Line 
	...
	if ((state == "hookshot") && (hs_inst != noone)) {
		...
		hs_angle = degtorad(point_direction(obj_player.x, obj_player.y, obj_hs_collider.x, obj_hs_collider.y)); //get the angle of the hs and convert it from degrees to radian.
		...
		for(var i = 0; i < hs_length; i += hs_width) //Run a loop so we draw every hs-segment
		{
		     ...
		     draw_sprite_ext(spr_hs_text, 0, obj_player.x + ( cos(hs_angle) * i ) + ( cos(hs_angle) * (hs_width/2) ), (obj_player.y - (sin(hs_angle) * i )) + (sin( hs_angle ) * (hs_width/2) ), 1, 1, (radtodeg(hs_angle) - 90), c_white, 1);
		}
	}
